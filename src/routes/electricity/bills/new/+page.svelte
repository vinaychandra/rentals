<script>
    ...
    $effect(() => {
        ...
        const meterClients = allClients.filter(
            (c) => c.MeterID === selectedMeterId && c.Status === "Active",
        );
        readings = meterClients.map((c) => {
            const clientReadings = allPastReadings.filter(
                (r) => r.ClientID === c.ID,
            );
            const lastReading =
                clientReadings.length > 0
                    ? clientReadings[clientReadings.length - 1].CurrentReading
                    : "";
            return {
                clientId: c.ID,
                clientName: c.Name,
    ...
    let calculations = $derived.by(() => {
        ...
        return items.map((item) => ({
            ...
            share:
                totalUnits > 0
                    ? Math.round((item.units / totalUnits) * total)
                    : 0,
        }));
    });
    ...
    async function saveBill() {
        ...
        try {
            ...
            const items = readingRows.map((row) => {
                ...
                const tUnits = readingRows.reduce(
                    (s, r) => s + Number(r[5]),
                    0,
                );
                const share =
                    tUnits > 0 ? Math.round((units / tUnits) * total) : 0;
                return [
                    generateId(),
                    billId,
                    row[2],
...
>
            ...
            {#if meters.length === 0}
                <p class="mt-1 text-sm text-gray-400">
                    No active meters. <a
                        ...
                        class="text-gray-600 underline">Add one first</a
                    >.
                </p>
            {:else}
                <select
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4">
        ...
        {:else if readings.length === 0}
            <p class="text-gray-400 text-sm">
                No active submeters for this meter. <a
                    href="{base}/electricity"
                    class="text-gray-600 underline">Add some first</a
                >.
            </p>
        {:else}
            <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div>
                                ...
                                <input
                                    id="prev-{i}"
                                    type="number"
                                    bind:value={readings[i].previousReading}
                                    class="mt-1 w-full rounded-md border border-gray-300 px-2 py-1.5 text-sm focus:border-gray-500 focus:outline-none"
                                />
                            </div>
                            <div>
                                <label
                                ...
                                <input
                                    id="curr-{i}"
                                    type="number"
                                    bind:value={readings[i].currentReading}
                                    class="mt-1 w-full rounded-md border border-gray-300 px-2 py-1.5 text-sm focus:border-gray-500 focus:outline-none"
                                />
                            </div>
                            <div>
                                ...
                                <p
                                    class="mt-1 text-sm font-medium text-gray-700 py-1.5"
                                >
                                    {calculations[i]?.units || 0} units — {formatINR(
                                        calculations[i]?.share || 0,
                                    )}
                                </p>
                            </div>
                        </div>
            ...
            {#if billAmount}
                <div
                    class="mt-4 pt-3 border-t border-gray-200 flex items-center justify-between text-sm"
                >
                    <span class="text-gray-500">Total: {totalUnits} units</span>
                    <span
                        class="font-medium {totalShare === Number(billAmount)
                            ? 'text-green-600'
                            : 'text-orange-500'}"
                    >
                        Allocated: {formatINR(totalShare)} / {formatINR(
                            Number(billAmount),
                        )}
                        {#if totalShare !== Number(billAmount) && totalUnits > 0}
                            <span class="text-xs text-gray-400"
                                >(rounding diff: {formatINR(
                                    Number(billAmount) - totalShare,
                                )})</span
                            >
                        {/if}
                    </span>
    ...
    <div class="flex justify-end">
        <button
            ...
            class="bg-gray-800 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
            Save Bill
        </button>
    </div>
