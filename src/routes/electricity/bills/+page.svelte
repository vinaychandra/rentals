<script>
    ...
    async function loadAll() {
        $loading = true;
        try {
            const [meterData, billData, clientData, readingData, paymentData] =
                await Promise.all([
    ...
    async function savePayment() {
        ...
        try {
            const rowNum = await findRowById(
                SHEET_NAMES.ELECTRICITY_PAYMENTS,
                payingRecord.ID,
            );
            if (rowNum === -1) throw new Error("Payment record not found");

            const newPaid =
                Number(payingRecord.AmountPaid || 0) + Number(payForm.amount);
            ...
            await updateRange(range, [
                [
                    payingRecord.ID,
                    payingRecord.BillID,
                    payingRecord.ClientID,
                    payingRecord.AmountDue,
                    String(newPaid),
                    payForm.method,
                    payForm.date,
                    status,
    ...
    >
        ...
        >
            <h2 class="text-lg font-semibold mb-4">
                Record Payment — {clientName(payingRecord.ClientID)}
            </h2>
            <p class="text-sm text-gray-500 mb-4">
                Due: {formatINR(Number(payingRecord.AmountDue))} | Paid: {formatINR(
                    Number(payingRecord.AmountPaid || 0),
                )} | Remaining: {formatINR(
                    Number(payingRecord.AmountDue) -
                        Number(payingRecord.AmountPaid || 0),
                )}
            </p>
            <form
...
{:else}
    <div class="space-y-4">
        {#each bills as bill}
            ...
            {@const totalPaid = bPayments.reduce(
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4">
                ...
                <div class="border-t border-gray-100 pt-2 space-y-2">
                    {#each bPayments as payment}
                        {@const reading = bReadings.find(
                            (r) => r.ClientID === payment.ClientID,
                        )}
                        {@const remaining =
                        ...
                        >
                            <div class="min-w-0 flex-1">
                                <span class="font-medium text-gray-700"
                                    >{clientName(payment.ClientID)}</span
                                >
                                {#if reading}
                                    <span class="text-gray-400 ml-1"
                                        >({reading.Units} units)</span
                                    >
                                {/if}
                            ...
                            <div class="flex items-center gap-2 ml-2 shrink-0">
                                ...
                                {:else if payment.Status === "Partial"}
                                    ...
                                    <button
                                        ...
                                        class="text-xs px-2 py-1 rounded bg-gray-100 text-gray-600 hover:bg-gray-200 cursor-pointer"
                                    >
