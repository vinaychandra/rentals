<script>
	...
	let form = $state({ name: '', address: '', monthlyRent: '', tenantName: '', tenantContact: '', status: 'Active', notes: '' });

	const HEADERS = ['ID', 'Name', 'Address', 'MonthlyRent', 'TenantName', 'TenantContact', 'Status', 'Notes'];
	...
	async function loadProperties() {
		$loading = true;
		try {
			const { rows } = await readSheetAsObjects(SHEET_NAMES.PROPERTIES);
			properties = rows;
		} catch (e) {
	...
	function resetForm() {
		form = { name: '', address: '', monthlyRent: '', tenantName: '', tenantContact: '', status: 'Active', notes: '' };
		editingId = null;
		showForm = false;
	}
	...
	async function saveProperty() {
		...
		try {
			if (editingId) {
				const rowNum = await findRowById(SHEET_NAMES.PROPERTIES, editingId);
				if (rowNum === -1) throw new Error('Property not found');
				const range = `${SHEET_NAMES.PROPERTIES}!A${rowNum}:H${rowNum}`;
				await updateRange(range, [[editingId, form.name, form.address, form.monthlyRent, form.tenantName, form.tenantContact, form.status, form.notes]]);
			} else {
				const id = generateId();
				await appendRows(SHEET_NAMES.PROPERTIES, [[id, form.name, form.address, form.monthlyRent, form.tenantName, form.tenantContact, form.status, form.notes]]);
			}
			resetForm();
...
{#if showForm}
	<div class="fixed inset-0 bg-black/40 z-40 flex items-start justify-center pt-20 px-4" role="presentation" onclick={() => resetForm()}>
		<div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6" role="presentation" onclick={(e) => e.stopPropagation()}>
			<h2 class="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Add'} Property</h2>
			<form onsubmit={(e) => { e.preventDefault(); saveProperty(); }} class="space-y-3">
				<div>
				...
				<div>
					...
					<input id="address" bind:value={form.address} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none" />
				</div>
				<div>
					<label for="rent" class="block text-sm font-medium text-gray-700">Monthly Rent (₹) *</label>
					<input id="rent" type="number" bind:value={form.monthlyRent} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none" />
				</div>
				<div class="grid grid-cols-2 gap-3">
					<div>
						<label for="tenant" class="block text-sm font-medium text-gray-700">Tenant Name</label>
						<input id="tenant" bind:value={form.tenantName} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none" />
					</div>
					<div>
						<label for="contact" class="block text-sm font-medium text-gray-700">Contact</label>
						<input id="contact" bind:value={form.tenantContact} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none" />
					</div>
				</div>
				<div>
...
{:else if properties.length === 0}
	<p class="text-gray-400 text-sm">No properties yet. Add one to get started.</p>
{:else}
	<div class="space-y-3">
				...
				<div class="flex items-start justify-between">
					<div class="min-w-0 flex-1">
						<div class="flex items-center gap-2 flex-wrap">
							...
							<span class="text-xs px-2 py-0.5 rounded-full {prop.Status === 'Active' ? 'bg-green-100 text-green-700' : prop.Status === 'Vacant' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'}">
								{prop.Status}
							</span>
						</div>
						{#if prop.Address}
							<p class="text-sm text-gray-500 mt-1">{prop.Address}</p>
						{/if}
						<p class="text-sm font-medium text-gray-700 mt-1">{formatINR(Number(prop.MonthlyRent))}/month</p>
						{#if prop.TenantName}
							<p class="text-sm text-gray-500 mt-1">Tenant: {prop.TenantName} {prop.TenantContact ? `(${prop.TenantContact})` : ''}</p>
						{/if}
					</div>
					<div class="flex items-center gap-1 ml-2 shrink-0">
						...
						<button onclick={() => editProperty(prop)} class="text-xs px-2 py-1.5 text-gray-400 hover:text-gray-700 cursor-pointer">Edit</button>
						<button onclick={() => deleteProperty(prop.ID)} class="text-xs px-2 py-1.5 text-red-400 hover:text-red-600 cursor-pointer">Delete</button>
					</div>
				</div>
			</div>
		{/each}
